...
def replay_from_simulation(
    ...
    while True:
        frame = _frame(state)
        decisions = _explain_all(state, rng, options)
...
def _explain_all(state: rules.GameState, rng: random.Random, options: Options) -> dict:
    return {
...
def main() -> None:
    ...
    if args.simulate:
        replay = replay_from_simulation(args.seed, args.opponents, max_turns=args.max_turns)
